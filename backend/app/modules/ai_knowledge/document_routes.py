...
@router.delete("/api/v1/documents/{document_id}",operation_id="deleteDocument")
async def delete(document_id:UUID,request:Request,user:Annotated[AuthenticatedUser,Depends(require_permissions("knowledge:write"))],x=Depends(deps)):
 d=await owned(x[0],x[1],user,document_id,True);d.status="deleted";d.deleted_at=datetime.now(UTC);return SuccessResponse(data={},request_id=request.state.request_id,timestamp=datetime.now(UTC))
...
async def publish(document_id:UUID,request:Request,user:Annotated[AuthenticatedUser,Depends(require_permissions("knowledge:publish"))],x=Depends(deps)):return SuccessResponse(data=doc(await transition(document_id,user,x,"published")),request_id=request.state.request_id,timestamp=datetime.now(UTC))
@router.post("/api/v1/documents/{document_id}/deactivate",operation_id="deactivateDocument")
async def deactivate(document_id:UUID,request:Request,user:Annotated[AuthenticatedUser,Depends(require_permissions("knowledge:publish"))],x=Depends(deps)):return SuccessResponse(data=doc(await transition(document_id,user,x,"inactive")),request_id=request.state.request_id,timestamp=datetime.now(UTC))
...
async def retry(job_id:UUID,request:Request,user:Annotated[AuthenticatedUser,Depends(require_permissions("knowledge:write"))],x=Depends(deps)):
 ...
 await owned(x[0],x[1],user,j.document_id,True);j.stage="queued";j.progress=0;j.attempt+=1;j.error_code=None;return SuccessResponse(data={"id":j.id,"stage":j.stage,"attempt":j.attempt},request_id=request.state.request_id,timestamp=datetime.now(UTC))
