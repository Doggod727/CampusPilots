...
def test_m1_revision_chain_and_schema_objects_are_frozen():
    revision = (ROOT / "migrations" / "versions" / "0007_ai_knowledge_schema.py").read_text(encoding="utf-8")
    sql = (ROOT / "migrations" / "versions" / "0007_ai_knowledge_schema.sql").read_text(encoding="utf-8")
    assert 'revision = "0007_ai_knowledge_schema"' in revision
    assert 'down_revision = "0006_agent_runtime_delivery"' in revision
